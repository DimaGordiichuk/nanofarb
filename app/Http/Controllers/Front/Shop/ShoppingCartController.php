...
class ShoppingCartController extends Controller
{
     ...
     * @return int
     */
    public function remove(Request $request, $id,  $amount = null, $color)
    {
        $product = Product::isPublish()->findOrFail($id);
...
//            Cookie::queue(Cookie::forget('uuid'));
        ...
        $identyUuid = Cookie::get('uuid') ? Cookie::get('uuid') : null;
...
//        })->sum();
        ...
        $sales = Sale::isPublish()
            ...
            ->where(function ($qSales) use ($promoCodesId, $productsIds, $categoriesIdCartProducts) {
                $qSales
                    // бесплатная доставка
                    ...
                    ->orWhere(function ($qSales2) use ($promoCodesId) {
                     ...
                     // скидка на товары
                    ->orWhere(function ($qSales2) use ($productsIds, $categoriesIdCartProducts) {
                        $qSales2->where('type', Sale::TYPE_PRODUCT)
                            ->whereHas('products', function ($products) use ($productsIds) {
                    ...
                    ->orWhere(function ($qSales2) use ($productsIds, $promoCodesId, $categoriesIdCartProducts) {
                        $qSales2->where('type', Sale::TYPE_PROM_CODE_PRODUCT)
                            ->whereHas('promoCodes', function ($qPromoCodes) use ($promoCodesId) {
                                $qPromoCodes->isActive()->whereIn('code', $promoCodesId);
                            })->where(function ($qSales3) use ($productsIds, $categoriesIdCartProducts) {
                                $qSales3->whereHas('products', function ($products) use ($productsIds, $categoriesIdCartProducts) {
                                    $products->whereIn('model_id', $productsIds);
                                })->orWhereHas('terms', function ($terms) use ($categoriesIdCartProducts) {
                                    $terms->whereIn('model_id', $categoriesIdCartProducts);
                                });
...
//            ->limit(10)->get();
        ...
        return Product::withBase()->isPublish()
            ->when(count($productsInCart), function ($p) use ($productsInCart,$categoriesIdCartProducts,$productsIdInCart) {
